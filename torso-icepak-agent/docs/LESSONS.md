...
**HOWEVER — the bar for SAYING "done" is bar 2, not bar 1.** Same day
user reinforced: "I suggest you always double check before you say
'done'. At least you need to have a clean run to say that." So
engineering convergence is a *checkpoint* ("we're close, let's verify"),
not a stopping condition. Always run long enough / escalate solver
settings until you get the clean numerically-converged run before
...
### Run PyAEDT externally, NOT via AEDT's Tools->Run Script

`Tools -> Run Script` inside the AEDT GUI uses AEDT's bundled interpreter,
which can be older and stricter than your external Python. Symptom:

```
SyntaxError: Non-ASCII character '\xe2' in file ... line 20,
  but no encoding declared; see http://www.python.org/peps/pep-0263.html
```

This is a Python-2-era error message and is impossible on Python 3.14.
Means the script ran on AEDT's internal interpreter, not yours.

**Rule:** PyAEDT scripts always run with the external Python 3.14
interpreter from a regular terminal:

```powershell
& "C:/Users/Jiong Chen/AppData/Local/Python/pythoncore-3.14-64/python.exe" `
   scripts/whatever.py args...
```

Also: keep all PyAEDT scripts in this repo ASCII-only with a
`# -*- coding: utf-8 -*-` header, so they survive the older-Python case
even if someone runs them through `Tools -> Run Script` by accident.

### PowerShell long-path multi-word paths

When a `.aedt` lives at a path with spaces (e.g. `70mm case`) and you
type a long command that wraps to a second line in PowerShell, the
shell treats the newline inside the quoted string as part of the path
and you get `file not found`. Workarounds, in order of preference:

1. `cd` into the project directory first, then use a short relative
   path:
   ```powershell
   cd "C:/Users/Jiong Chen/Documents/tmp_sim_files/_FIG4_P0_torso/70mm case"
   & "C:/Users/.../python.exe" scripts/foo.py "outputs/v3_fixed/<name>.aedt"
   ```
2. Use the PowerShell line-continuation backtick `` ` `` at end of each
   non-final line (NOT a bare newline).
3. Paste the whole command on a single continuous line.
...
### Model-Based Flow Initialization is a TRAP for fan-driven flows

`Setup1 → 'Solution Initialization - Use Model Based Flow Initialization' = True`
is sold as a way to start the solver closer to the converged answer
(a quick Laplace pre-solve seeds the velocity field). For *simple
external-flow* problems it can shave ~100 iters off the front of a run.

**For systems with active fans and porous resistances (Grilles), it
produces a non-physical initial field that the iterative solver
amplifies, not damps.** Observed empirically 2026-05-20 on the noduct
design: with model-based init on, MassFlow started at 210 g/s and
climbed to 490 g/s by iter 45, vs the converged value of 7.7 g/s. The
Laplace pre-solve does not know about the fan curves or grille
resistance, so it solves an unconstrained potential-flow problem that
overshoots wildly. The k-ω SST solver then converges toward a
*different* steady state with the wrong fan operating point.

**Rule:** for any project containing Fan boundaries or Grille porous
resistances, keep `Use Model Based Flow Initialization = false` and
let the solver develop the flow from rest — slower startup, correct
answer.
